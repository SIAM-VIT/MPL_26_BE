...
MASK_KEYS_TS = {"timer_start_time", "created_at", "finished_at",
                "first_solved_at", "last_submission_at"}
...
def step(name, method, path, body=None, headers=None, params=None, raw=False):
    try:
        r = requests.request(method, BASE + path, json=body,
                             headers=headers or {}, params=params, timeout=90)
        if raw:
...
Q2_RAW_FLOAT = "p = float(input())\nr = float(input())\nt = float(input())\nprint(p * (1 + r) ** t)\n"
Q3_TWOSUM = ("n = int(input())\na = list(map(int, input().split()))\ntarget = int(input())\n"
...
# ── auth ─────────────────────────────────────────────────────────────────────
step("login_bad", "POST", "/api/auth/login",
     body={"name": "Team Alpha", "passcode": "nope"})
_, login = step("login", "POST", "/api/auth/login",
                body={"name": "Team Alpha", "passcode": "alpha123"})
TOKEN = login["session_token"]
AUTH = {"X-Team-Token": TOKEN}

step("questions_noauth", "GET", "/api/main/questions")
step("questions_badauth", "GET", "/api/main/questions",
     headers={"X-Team-Token": "bogus"})
...
step("submit_math_rawfloat", "POST", "/api/main/submit", sub(2, "python", Q2_RAW_FLOAT), headers=AUTH)
...
step("create_boost_q", "POST", "/api/admin/questions",
     body={"title": "Boost: ASCII sum", "description": "Sum the bytes.",
           "test_cases": "[]", "type": "TIME_BOOST", "difficulty": "EASY",
           "reward_value": 300}, headers=ADMIN)
step("assign_boost", "POST", "/api/admin/teams/2/assign-boost",
     body={"question_id": 4}, headers=ADMIN)
step("assign_boost_dup", "POST", "/api/admin/teams/2/assign-boost",
     body={"question_id": 4}, headers=ADMIN)
step("beta_status", "GET", "/api/teams/2/status")
step("mark_boost_solved", "POST", "/api/admin/review/mark-solved",
     body={"team_id": 2, "question_id": 4}, headers=ADMIN)
step("create_challenge_q", "POST", "/api/admin/questions",
     body={"title": "Challenge: reverse", "description": "Reverse it.",
           "test_cases": "[]", "type": "CHALLENGE", "difficulty": "HARD",
           "reward_value": 250}, headers=ADMIN)
step("create_session", "POST", "/api/admin/challenge/create",
     body={"question_id": 5, "team1_id": 1, "team2_id": 2}, headers=ADMIN)
step("alpha_status_session", "GET", "/api/teams/1/status")
step("mark_challenge_won", "POST", "/api/admin/review/mark-solved",
     body={"team_id": 1, "question_id": 5}, headers=ADMIN)
step("mark_main_info", "POST", "/api/admin/review/mark-solved",
     body={"team_id": 1, "question_id": 1}, headers=ADMIN)
step("leaderboard_final", "GET", "/api/admin/leaderboard")
step("submit_nonmain_400", "POST", "/api/main/submit", sub(4, "python", "print(1)\n"), headers=AUTH)

# ── test-case CRUD ───────────────────────────────────────────────────────────
step("add_testcase", "POST", "/api/admin/questions/1/test-cases",
     body=[{"stdin": "7", "expected_output": "28", "is_hidden": False,
            "weight": 1.0, "position": 9}],
     params={"replace": "false"}, headers=ADMIN)
...
step("add_testcase_q404", "POST", "/api/admin/questions/999/test-cases",
     body=[], headers=ADMIN)

# ── token rotation ───────────────────────────────────────────────────────────
_, login2 = step("relogin", "POST", "/api/auth/login",
                 body={"name": "Team Alpha", "passcode": "alpha123"})
step("clock_oldtoken_401", "GET", "/api/main/clock", headers=AUTH)
step("clock_newtoken", "GET", "/api/main/clock",
     headers={"X-Team-Token": login2["session_token"]})
