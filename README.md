...
Pairs with the **MPL-FE** repo, which this API serves at `/ui` for
single-origin running (see §5).
...
```
app/                       backend package (every module ≤ 200 lines; see REFACTOR_NOTES.md)
  main.py                  FastAPI app, CORS, /ui mount (see FRONTEND_DIR), lifespan
  ...
  routes/
    ...
    main.py                thin MAIN-round routes (logic in services/)
    questions.py           public question reads (hidden tests stripped)
    teams.py               legacy status + time-remaining (IDOR, see §2)
    admin/                 deps (passcode), teams, questions, testcases,
                           submissions(+rejudge), leaderboard, legacy
  services/
    judge/                 base DTOs, mock.py (dev), client.py + polling.py
                           (Judge0 CE), selected by JUDGE_BACKEND
    scoring.py             output comparison (4 modes) + partial-credit maths
    judging.py             run/submit pipeline · validation.py request guards
    access.py              team token auth + clock helpers
    progress.py            best-score accounting · results.py / rejudge.py
    arena.py / questions.py

seed.py                    seeding entry point (via the admin API)
seeding/                   starter code + question payloads
reset_db.py                drop and recreate all tables (dev)
create_team.py             CLI to add one team
tools/smoke_test.py        65-step end-to-end regression against a live API
REFACTOR_NOTES.md          old→new module map for the split backend
...
You need: Python 3.11+ and Git. No Docker, no Postgres for local dev.
...
Always use `python -m pip` (never bare `pip`) and never copy a `.venv`
between folders — create it fresh on each machine.

### Switch the database to SQLite (one edit)

`.env` defaults to Postgres. For local dev, open `.env` and swap:
...
Everything else can stay as-is (`JUDGE_BACKEND=mock` = run code locally,
no Docker).
...
Then start the server (**keep this window open**):

```powershell
uvicorn app.main:app --port 8000
```

And in a **second** terminal (venv activated, server running):

```powershell
python seed.py         # 3 demo teams + 3 demo questions (talks to the API)
```
...
### Every time (daily restart)

Activate the venv, start the server. Skip `reset_db.py`/`seed.py` — data
persists in `mpl.db`.
...
## 5. Pairing with the frontend (MPL-FE)

This repo serves no pages by itself — `/ui` appears only when the API is
pointed at a frontend checkout via `FRONTEND_DIR` (empty = `./frontend`,
the classic single-repo layout; a missing folder means clean API-only mode).

### Recommended: single origin (event day + normal dev)
...
Start the API (§4) and open `http://localhost:8000/ui/landing.html`.
One process, one URL, no CORS.

### Alternative: separate static server

Serve the MPL-FE folder with Live Server / `python -m http.server 5500`.
The pages call `http://localhost:8000/api` automatically — just keep the
API on port 8000.
...
|---|---|---|
| `DATABASE_URL` | postgres… | SQLAlchemy async URL (switch to SQLite for dev, §4) |
| `ADMIN_PASSCODE` | `admin123` | **Change before the event** |
| `FRONTEND_DIR` | empty (`./frontend`) | folder served at `/ui`; e.g. `../MPL-FE` when split |
...
Open the console (`/ui/main.html` once the frontend is paired, §5), log in,
and submit a known-good solution in **Python, C, C++ and Java**. All four
must come back `PASSED`.
...
| Every submit returns `ERROR` | Client/contract mismatch | Check the API log for the Judge0 HTTP error — that message will point at the fix in `app/services/judge/` |
...
`main` is the stable branch. Feature work goes on short-lived branches,
merged by pull request after `tools/smoke_test.py` passes against a live
server (65 steps, must exit 0):

```powershell
uvicorn app.main:app --port 8000
python tools/smoke_test.py http://localhost:8000
```

Keep every `app/` module small and single-purpose (see `REFACTOR_NOTES.md`
for the module map). The frontend lives in its own repo — this repo must
never grow a second copy of it; pair via `FRONTEND_DIR` (§5) instead.
